...
WinHttpRedirectProxy/
├── common/                          # 公共 IPC 基础层
│   ├── ipc_pipe_transport.hpp       # 管道读写 / 创建 / 连接工具
│   ├── ipc_control.hpp              # 控制协议定义 + Send 函数
│   └── ipc_memory.hpp               # 内存协议定义 + Send 函数
├── controller/                      # 控制器模块 (GUI + CLI)
│   ├── controller_state.*           # AgentSession / ControllerState
│   ├── controller_session.*         # 会话线程 + 消息处理
│   ├── controller_window.*          # WndProc + 消息分发
│   ├── controller_window_layout.*   # 控件创建 / 布局
│   ├── controller_view_model.*      # 显示行构建
│   ├── controller_gui.*             # RunGui 入口
│   ├── controller_pipe_discovery.*  # 内存 IPC 管道枚举
│   ├── controller.cpp               # GUI 控制器 EXE 入口
│   └── cli_controller.cpp           # CLI 控制器 EXE 入口
├── memory/                          # 内存访问代理模块
│   ├── memory_runtime.hpp           # 公共 API: Start / StopMemoryIpcRuntime
│   ├── memory_runtime_internal.hpp  # 内部类型声明
│   ├── memory_access_core.cpp       # 区域缓存 / VEH / SEH 读写
│   ├── memory_access_requests.cpp   # 请求验证 / reply 组装
│   ├── memory_shared.cpp            # 共享内存会话管理
│   ├── memory_server.cpp            # 线程主循环 / VEH 注册
│   └── memory_client.cpp            # 内存 CLI 客户端入口
├── proxy/                           # DLL 壳模块
│   ├── forward_exports.hpp          # 82 个 WinHTTP 导出转发声明
│   ├── redirect_proxy.hpp           # 聚合头
│   ├── redirect_runtime.*           # DLL 侧运行时 / Agent 线程
│   └── dllmain.cpp                  # 极薄 DLL 入口
├── pch.h / pch.cpp                  # 预编译头
├── build.bat                        # 编译脚本
├── deploy.bat                       # 部署到目标目录
└── remove.bat                       # 从目标目录移除
...
| `winhttp_cli_controller.exe` | CLI 控制器 |
| `winhttp_memory_client.exe` | 命令行内存读写工具 |
...
代理 DLL 会在目标进程内额外开一个按 `PID` 区分的 Named Pipe：
...
- **模块划分**：`common/`（IPC 基础）→ `controller/`（GUI/CLI 控制器）→ `memory/`（内存访问代理）→ `proxy/`（DLL 壳）
- **IPC**：Windows Named Pipe，控制管道 `\\.\pipe\WinHttpRedirectProxyControl`，内存管道 `\\.\pipe\WinHttpRedirectProxyMemory-<pid>`
- **控制方式**：由外部 GUI/CLI 控制器选择进程并下发 DLL 路径
