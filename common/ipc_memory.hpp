...
#ifndef WINHTTP_REDIRECT_PROXY_MEMORY_MAX_PATH_CHARS
#define WINHTTP_REDIRECT_PROXY_MEMORY_MAX_PATH_CHARS 1024
#endif
...
namespace WinHttpRedirectMemoryIpc
{
    ...
    inline constexpr std::uint32_t kMaxModuleEntries = 256;
    inline constexpr std::uint32_t kModuleEntryNameChars = 260;

    struct ModuleEntry
    {
        std::uint64_t baseAddress;
        std::uint64_t sizeOfImage;
        wchar_t name[kModuleEntryNameChars];
    };

    struct ModuleEnumRequestPayload
    {
        std::uint64_t requestId;
    };

    struct ModuleEnumReplyPayload
    {
        std::uint64_t requestId;
        std::uint32_t status;
        std::uint32_t win32Error;
        std::uint32_t count;
        std::uint32_t reserved;
        ModuleEntry entries[kMaxModuleEntries];
    };

    struct LoadDllRequestPayload
    {
        std::uint64_t requestId;
        wchar_t dllPath[WINHTTP_REDIRECT_PROXY_MEMORY_MAX_PATH_CHARS];
    };
    ...
    struct LoadDllReplyPayload
    {
        ...
        std::uint32_t win32Error;
        wchar_t dllPath[WINHTTP_REDIRECT_PROXY_MEMORY_MAX_PATH_CHARS];
        wchar_t text[WINHTTP_REDIRECT_PROXY_MEMORY_MAX_PATH_CHARS];
    };
    ...
    inline bool SendModuleEnumRequest(
        HANDLE pipeHandle,
        std::uint32_t pid,
        std::uint64_t requestId)
    {
        ModuleEnumRequestPayload payload = {};
        payload.requestId = requestId;
        return WriteMessage(
            pipeHandle,
            MessageKind::ModuleEnumRequest,
            pid,
            &payload,
            static_cast<std::uint32_t>(sizeof(payload)));
    }

    inline bool SendModuleEnumReply(
        HANDLE pipeHandle,
        std::uint32_t pid,
        const ModuleEnumReplyPayload& payload)
    {
        return WriteMessage(
            pipeHandle,
            MessageKind::ModuleEnumReply,
            pid,
            &payload,
            static_cast<std::uint32_t>(sizeof(payload)));
    }

    inline bool SendLoadDllRequest(
        HANDLE pipeHandle,
        std::uint32_t pid,
        std::uint64_t requestId,
        const std::wstring& dllPath)
    {
        ...
        payload.requestId = requestId;
        wcsncpy_s(payload.dllPath, WINHTTP_REDIRECT_PROXY_MEMORY_MAX_PATH_CHARS, dllPath.c_str(), _TRUNCATE);
        return WriteMessage(
            pipeHandle,
    ...
    inline bool SendLoadDllReply(
        HANDLE pipeHandle,
        std::uint32_t pid,
        const LoadDllReplyPayload& payload)
    {
        return WriteMessage(
            pipeHandle,
